...
def data_after_attention(Data_dict, Neighbors_AfterCausal, n_train, n_valid, n_test, if_shuffle=False):
...
def dataset_generator(Cell_list, Traffic_dict, Traffic_all_cell, if_shuffle=False, cell_need=''):
    ...
    Input_dict = data_after_attention(Data_dict, Neighbors_AfterCausal, n_train, n_valid, n_test, if_shuffle=if_shuffle)
...
def final_model_train(Input_dict, TCNModel, num_blocks=7):
    ...
    Model.add_module('MLP', MLP(input_size = 21, hidden_size=7))
...
for cell in Traffic_Input:
    ...
    x_input = torch.tensor(Traffic_Input[cell]['X']['train'])

    prediction = np.zeros(336)
    prediction[:21] = Traffic[cell][:21]
    for time in range(44):
        x = x_input[time*7].to(torch.float32).cuda().reshape(1, 21, 2)
        x = Model(x)
